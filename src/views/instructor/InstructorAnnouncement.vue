<template>
    <div class="instructor-announcement-container">
        ...
        <div class="select-filter-block">
            <v-select
                ...
                item-text="title"
                v-model="messageType"
                @change="fetchInstructorAnnouncements"
            />
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.fetchInstructorAnnouncements(this.messageType);
        },
        methods: {
            getSchoolClasses() {
            ...
            fetchInstructorAnnouncements(type) {
                this.announcements = [];
                if (type === 'inbox') {
                    this.isLoading = true;
                    this.announceObj.roleType = '';
                    this.announceObj.classId = '';
                    AnnouncementService.getSchoolAnnouncements(this.school.id, 'INSTRUCTOR').then((res) => {
                        this.announcements = res;
                        this.isLoading = false;
                    }).catch((err) => {
                        this.$toast.error(err);
            ...
            openAnnouncementModal(mode, announce) {
                if (mode === 'create' && !this.announceObj.classId) {
...
<style lang="scss">
    .instructor-announcement-container {
        .select-filter-block {
            margin-left: 30px;
            .v-select {
                max-width: 150px;
